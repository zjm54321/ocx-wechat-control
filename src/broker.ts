...
import { CONTROL_OFF_TEXT, HELP_TEXT, OFFLINE_TEXT, PERMISSION_DENIED_TEXT, SerialQueue, Store, formatOutbound, formatRegistrationList, isPlainText, isValidRouteMetadata, parseInboundText, type Binding, type RoutedBinding, type WeixinInbound } from "./core"
...
export class BrokerService {
	...
	async handleRequest(request: Request): Promise<Response> {
		...
		if (body.method === "leave-root") {
			...
			try { return Response.json({ ok: true, binding: this.store.bind({ rootSessionId: body.rootSessionId, directory: body.directory, ownerInstance: body.instanceId, title: body.title }) }) }
			catch (error) { return Response.json({ error: error instanceof Error && error.message === "owner-live" ? "owner-live" : "registration-conflict" }, { status: 409 }) }
		}
		...
		if (body.method === "control-get") {
			...
			return Response.json({ ok: true, ...control, adapter: this.adapter.status(), registered: Boolean(binding), alias: binding?.alias ?? null, routeReady: Boolean(route.conversationId && route.contextToken), routable: Boolean(binding && binding.ownerInstance === body.instanceId && route.conversationId && route.contextToken) })
		}
		if (body.method === "back-global") return Response.json({ ok: true, ...this.store.setControl(false) })
		...
		if (body.method === "permission-denied-notice") {
			...
			if (!validId(body.permissionId) || !this.store.control().enabled || !this.store.route().contextToken || binding?.ownerInstance !== body.instanceId) return Response.json({ ok: true, handled: false })
	...
	private async requestInput(body: any): Promise<Response> {
		...
		const route = this.store.route()
		if (!route.conversationId || !route.contextToken || !control.enabled || typeof body.question !== "string" || !isPlainText(body.question) || !body.question.trim() || body.question.length > 1500 || !Array.isArray(rawChoices) || rawChoices.length > 8 || rawChoices.some((value: unknown) => typeof value !== "string" || !isPlainText(value) || !value.trim() || value.length > 120 || /[\r\n]/.test(value))) return Response.json({ error: "invalid-request-input" }, { status: 409 })
		...
		if (!this.store.openCheckpoint({ checkpointId, requestKey: body.requestKey, root: binding.rootSessionId, owner: body.instanceId, alias: binding.alias, question: body.question, choices: choices as string[], revision: control.revision })) return Response.json({ error: "checkpoint-already-active" }, { status: 409 })
