...
export interface BrokerOptions { callbackTimeoutMs?: number; typing?: TypingCoordinator }
...
export class BrokerService {
	...
	constructor(readonly store: Store, readonly adapter: WeChatAdapter, readonly sharedSecret: string, readonly workerToken: string, private readonly fetcher: Fetch = fetch, options: BrokerOptions = {}) { this.callbackTimeoutMs = clampCallbackTimeout(options.callbackTimeoutMs); this.typing = options.typing }
	...
	async handleRequest(request: Request): Promise<Response> {
		...
		if (body.method === "leave-root") {
			...
			const instance = this.store.instance(body.instanceId); if (!instance || !(await this.callbackHealth(instance.endpoint, body.instanceToken, body.rootSessionId))) return Response.json({ error: "callback-session-unreachable" }, { status: 409 })
		...
		if (body.method === "observe-status") {
			...
			if (!binding || binding.ownerInstance !== body.instanceId || (body.status !== "busy" && body.status !== "retry" && body.status !== "idle")) return Response.json({ ok: true, observed: false })
			...
			try { const response = await this.fetcher(`${instance.endpoint}/runtime-status`, { method: "POST", headers: callbackHeaders(this.sharedSecret, body.instanceToken as string), body: JSON.stringify({ rootSessionId: binding.rootSessionId, rootSessionIds: [binding.rootSessionId], directory: binding.directory }), signal: AbortSignal.timeout(3000) }); const value = object(await response.json()), rows = value?.statuses; const row = Array.isArray(rows) ? object(rows[0]) : undefined; if (response.ok && row?.rootSessionId === binding.rootSessionId && (row.status === "BUSY" || row.status === "RETRY" || row.status === "IDLE")) status = row.status } catch {}
		...
		if (body.method === "permission-denied-notice") {
			const binding = validId(body.rootSessionId) ? this.store.bindingForRoot(body.rootSessionId) : undefined
			if (!validId(body.permissionId) || !this.store.control().enabled || !this.store.route().contextToken || !binding || binding.ownerInstance !== body.instanceId) return Response.json({ ok: true, handled: false })
	...
	async handleInbound(message: WeixinInbound): Promise<{ ok: boolean; reason?: string }> {
		...
		if (!binding) { this.store.markUnknown(message.id, "invalid-alias"); await this.safeSystemReply(message, "unauthorized", "未授权的会话编号。"); return { ok: false, reason: "unauthorized" } }
	...
	private async admitPrompt(message: WeixinInbound, binding: Binding, instance: { endpoint: string; instanceToken: string }, text: string): Promise<{ ok: boolean; reason?: string }> {
		return this.queue.run(binding.rootSessionId, async () => {
			...
			if (!control.enabled || this.store.bindingForRoot(binding.rootSessionId)?.ownerInstance !== binding.ownerInstance) { this.store.markUnknown(message.id, "control-changed-before-admission"); return { ok: false, reason: "control-disabled" } }
			const messageId = promptMessageIdForInbound(message.id), submission = this.store.claimPromptSubmission({ submissionId: message.id, inboundId: message.id, root: binding.rootSessionId, owner: binding.ownerInstance, alias: binding.alias, messageId, body: text, revision: control.revision })
			...
			try {
				...
				if (result?.certainty === "REJECTED") { this.store.finishPromptSubmission(message.id, "REJECTED", typeof result.error === "string" ? result.error : undefined, true); this.store.markUnknown(message.id, "prompt-rejected-no-effect"); return { ok: false, reason: "prompt-rejected" } }
	...
	private async nativeRequestOpen(body: JsonObject): Promise<Response> {
		...
		if (!binding || binding.ownerInstance !== body.instanceId || !validId(body.requestId) || !validId(body.requestKey) || (kind !== "QUESTION" && kind !== "PERMISSION")) return Response.json({ error: "native-request-unavailable" }, { status: 409 })
	...
	private nativeRequestTerminal(body: JsonObject): Response {
		...
		if (!binding || binding.ownerInstance !== body.instanceId || !control.enabled || !route.conversationId || !route.contextToken || !validId(body.requestId) || (body.state !== "RESOLVED" && body.state !== "REJECTED")) return Response.json({ error: "invalid-native-terminal" }, { status: 400 })
	...
	private async wechatReply(body: JsonObject): Promise<Response> {
		...
		if (!binding || binding.ownerInstance !== body.instanceId || !validId(body.callId) || !boundedText(body.text, 4000) || !this.store.control().enabled || !route.conversationId || !route.contextToken || this.adapter.status() !== "Ready") return Response.json({ error: "reply-unavailable" }, { status: 409 })
	...
	private async callbackHealth(endpoint: string, token: string, rootSessionId?: string): Promise<boolean> { try { const response = await this.fetcher(`${endpoint}/health`, { method: "POST", headers: callbackHeaders(this.sharedSecret, token), body: JSON.stringify({ rootSessionId }), signal: AbortSignal.timeout(3000) }); return response.ok } catch { return false } }
